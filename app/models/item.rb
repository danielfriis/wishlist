  ...
  def self.sort_general(general, current_user)
    if general == "recent"
      ...
      with_pictures
          ...
          .select("items.*, (count(distinct(case when impressions.created_at BETWEEN '#{start_date}' AND '#{end_date}' then ip_address end)) + (count(distinct(case when wishes.created_at BETWEEN '#{start_date}' AND '#{end_date}' then wishes.id end)) * 2)) as counter, impressionable_id")
