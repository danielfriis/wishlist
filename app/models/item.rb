  ...
  def self.sort_general(general, current_user)
    if general == "recent"
      ...
      with_pictures
          ...
          .select("items.*, (count(distinct(case when (impressions.created_at BETWEEN '#{start_date}' AND '#{end_date}') then ip_address end)) + count(wishes.id) * 10) as counter, impressionable_id")
