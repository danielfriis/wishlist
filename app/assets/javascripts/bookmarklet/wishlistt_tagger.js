...
var host = 'localhost:3000', prefix = 'wishlistt-bookmarklet-tagger-',
    iframe  = {obj:null, id:prefix+'iframe', win: null, url:'http://'+host+'/bookmarklet'},
	marker  = {obj:null, id:prefix+'marker'}, // image marker
    // style   = {url:'http://fancy/_ui/bookmarklet/tagger/css/bookmarklet.css?v=0007', id:prefix+'css'},
...
extend(iframe, {
	show : function(){
		...
		for(i=0,c=im.length; i < c; i++){
			if(im[i].src && im[i].offsetWidth > 280 && im[i].offsetHeight > 280) {
...
(function(){
	...
	if(!iframe.obj) {
		factory.innerHTML = '<iframe id="'+iframe.id+'" allowtransparency="true" style="display:none;position:fixed;top:10px;right:10px;border:1px solid #4c515c;z-index:100001;margin:0;background:#eff1f7;width:289px;height:372px"></iframe>';
...
if(!window.wishlistt_bookmarklet) window.wishlistt_bookmarklet = {};
window.wishlistt_bookmarklet.tagger = tagger;
